...
십신 분포: ${sipStr}
지장간(여기·중기·정기): ${hide}
십이운성: ${pKeys.map((k) => `${k === 'year' ? '연' : k === 'month' ? '월' : k === 'day' ? '일' : '시'}${(dt[k] || {}).unseong || ''}`).join(' ')}
천간 관계: ${R.gan}
지지 관계: ${R.ji}
신살: ${R.sinsal}
용신(가장 필요한 기운, 1~2개): ${(saju.yongsin || []).join('·') || '-'}
부족 오행(작명 보충 후보, 용신과 별개): ${(saju.lacking || []).join('·') || '없음'}
...
2. 아래 순서로 **마크다운 섹션**을 만든다. 각 섹션 제목(##)은 반드시 신조어·밈·구어로 후킹하게(예: "답정너에 고구마 한 스푼", "월급루팡은 내 길이 아니야"). 명리 용어를 제목에 직접 노출하지 말 것.
   ...
   ## 방향·풍수(부족 오행 → 방위/지역)
   ## 마무리 응원
3. 각 섹션 본문은 2문단. **1문단**=구어체로 공감·캐릭터 묘사. **2문단**=반드시 아래 데이터의 구체 근거(간지·십신 한자병기 예 '편인(偏印)', 신살, 'OO충/반합/자형', 십이운성 예 '제왕·장생·묘')를 인용해 왜 그런지 설명하고, 이어서 행동 처방을 준다. 특히 일지·월지의 십이운성은 기운의 왕쇠(旺衰)로 성격·활력에 반드시 녹여라.
...
6. 용신은 데이터의 '용신'(1~2개)만 사용하고 '부족 오행'과 혼동하지 말 것. 대표 비유·요약과 본문에서 말하는 용신을 반드시 일치시켜라(서로 다른 오행을 용신이라 하지 말 것).
...
2. 아래 마크다운 섹션을 만든다. 각 ## 제목은 반드시 신조어·밈·구어로 후킹하게(명리 용어를 제목에 직접 노출 금지). 각 섹션 본문은 2~3문단으로 메인보다 깊게 — 1문단은 구어체 공감·캐릭터 묘사, 이어지는 문단은 반드시 아래 데이터의 구체 근거(간지·십신 한자병기 예 '편인(偏印)', 신살, 'OO충/반합/자형', 십이운성 예 '제왕·장생·묘', 대운·세운 간지)를 인용해 설명하고 행동 처방까지 준다. 십이운성(특히 일지·월지)의 왕쇠를 성격·활력 해석에 반드시 반영하라.
   ## (대표 한 줄 비유 — 한마디로)
   ## 타고난 성격·기질
   ## 오행 균형과 용신
   ## 십신이 말하는 적성
   ...
   ## 올해 운세 (세운 + 타로 1장)
   ## 방향·풍수 & 마무리 응원
3. 톤: 단정+긍정, 재치. 불행·질병·사망 단정 금지. 데이터에 없는 합충·신살은 지어내지 말 것. 용신은 데이터의 '용신'(1~2개)만 쓰고 '부족 오행'과 혼동 말 것 — 모든 섹션에서 용신을 일관되게 유지.${tu ? '\n4. 출생시각 미상 — 시주·시간 기반 해석 금지, 연·월·일 세 기둥만으로 풀이.' : ''}
...
// 실제로 쓰는 2음절 한글 이름 사전 — 자연스러움 판정 기준(없으면 순무·수모 류로 간주)
const REAL_NAMES = new Set((
  '민준 서준 도윤 예준 시우 하준 주원 지호 지후 준우 준서 도현 건우 현우 우진 선우 서진 연우 정우 승우 승현 시윤 유준 정민 이준 윤우 시후 지환 수호 은우 지우 재윤 시현 진우 민재 현준 준혁 지훈 도훈 태윤 동현 지성 성민 태현 준영 영민 현서 도경 시원 지원 우현 한결 준수 민규 태경 재원 동욱 재현 준호 민호 진호 태호 종현 형준 시온 가온 라온 하랑 이안 로운 주안 단우 윤재 채운 강민 광현 기현 남준 다온 대현 동건 동훈 명준 문수 백호 범준 병호 상민 상우 상현 석준 선재 성현 성준 성우 세준 세현 수현 승민 승준 승호 신우 영준 예성 예찬 온유 우빈 우성 원준 유빈 윤호 은찬 의준 인우 재희 정환 종우 주하 준희 지완 지율 진서 찬우 찬영 창민 채준 천우 태민 태우 하민 하온 한우 해준 혁준 현성 현민 형석 호준 효준 휘준 희찬 ' +
  '영수 영호 영철 영진 정호 성호 상호 재호 재민 종민 명수 광수 진수 태수 동주 동수 형우 준영 정훈 성훈 재훈 ' +
  '서연 서윤 서현 하윤 하은 민서 지유 윤서 채원 수아 다은 은서 예은 지아 수빈 예린 소율 예나 유나 시아 지윤 채은 다인 예원 가은 소윤 지안 서아 아인 하린 시은 유주 나윤 다윤 채아 윤아 소은 예진 수민 하율 서영 민지 현지 수연 가영 가연 가현 경민 고은 다연 다현 도연 도하 미소 미주 민경 민아 민주 보라 보민 새봄 서우 서율 선아 선영 설아 성은 세아 세은 세빈 소연 소영 소희 수경 수정 수지 슬기 승아 시연 신애 아라 아름 아윤 아현 연서 연수 연재 영주 예솔 예슬 예지 예주 우연 유경 유민 유진 윤정 은비 은수 은유 은정 은채 은하 이슬 정연 정원 정은 주아 주연 주영 주은 지민 지선 지수 지연 지현 진아 진영 채린 채영 채윤 초아 태희 하나 하늘 하영 한나 해나 현아 현주 혜린 혜원 혜진 효주 희수 희연 희주 보경 수진 유빈 다윤 가율 시현 라희 다희').trim().split(/\s+/)
);
// 자연스러움 — 결정적 로컬 점수 (실제 이름 사전 기준)
function localNat(r) {
  const nm = r.chars.map((c) => c.hangul).join('');
  if (REAL_NAMES.has(nm)) { // 실제 쓰는 이름
    let s = 92;
    if ((r.axes && r.axes.eumyang) >= 80) s += 4;
    if ((r.axes && r.axes.eum) >= 80) s += 4;
    return Math.min(100, s);
  }
  // 사전에 없는 조합(순무·수모·동유 류) → 음절 흔함 정도만 낮게
  const ic = window.Naming.isCommonSyll;
  const a = ic(r.chars[0].hangul), b = ic(r.chars[1].hangul);
  return (a && b) ? 58 : (a || b) ? 48 : 38;
}
